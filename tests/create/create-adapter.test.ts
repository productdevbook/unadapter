...
async function createTestAdapter<T extends Record<string, any> = Record<string, any>>(
  props: {
    ...
    adapter?: (...args: Parameters<CreateCustomAdapter>) => Partial<ReturnType<CreateCustomAdapter>>
  } = {
    config: {
      adapterId: "test-id",
  ...
  return testAdapter(getAuthTables, {
    ...options,
  ...
  describe("checking for the results of an adapter call, as well as the parameters passed into the adapter call", () => {
    describe("create", () => {
      ...
      it("should modify boolean type to 1 or 0 if the DB doesn't support it. And expect the result to be transformed back to boolean", async () => {
        // Testing true
        const createTRUEParameters: { data: { emailVerified: number } } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter({
              config: {
        ...
        // Testing false
        const createFALSEParameters: { data: { emailVerified: number } } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                config: {
      ...
      it("should modify JSON type to TEXT if the DB doesn't support it. And expect the result to be transformed back to JSON", async () => {
        const createJSONParameters: { data: { preferences: string } } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter({
              config: {
                supportsJSON: false,
        ...
        expect(createJSONParameters.data.preferences).toEqual('{"color":"blue","size":"large"}')
      })
      ...
      it("should modify date type to TEXT if the DB doesn't support it. And expect the result to be transformed back to date", async () => {
        ...
        const createDateParameters: { data: { createdAt: string } } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter({
              config: {
                supportsDates: false,
        ...
        expect(createDateParameters.data.createdAt).toEqual(testDate.toISOString())
      })

      it("should allow custom transform input", async () => {
        const createCustomTransformInputParameters: { data: { name: string } } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                config: {
        ...
        expect(createCustomTransformInputParameters.data).toHaveProperty("name")
        expect(createCustomTransformInputParameters.data.name).toEqual("TEST-NAME")
      })
      ...
      it("should allow custom transform output", async () => {
        ...
        expect(createCustomTransformOutputParameters.data).toHaveProperty("name")
        expect(createCustomTransformOutputParameters.data.name).toEqual("TEST-NAME")
      })
      ...
      it("should allow custom transform input and output", async () => {
        ...
        expect(createCustomTransformInputAndOutputParameters.data).toHaveProperty("name")
        expect(createCustomTransformInputAndOutputParameters.data.name).toEqual("TEST-NAME")
      })
      ...
      it("should expect the fields to be transformed into the correct field names if customized", async () => {
        const parameters: { data: any; select?: string[]; model: string } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                config: {
    ...
    describe("update", () => {
      ...
      it("should modify boolean type to 1 or 0 if the DB doesn't support it. And expect the result to be transformed back to boolean", async () => {
        // Testing true
        const updateTRUEParameters: { update: { emailVerified: number } } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                config: {
              ...
              const user: { emailVerified: boolean; id: string } = await adapter.create({
                model: "user",
                data: { emailVerified: false },
              })
              const res = await adapter.update({
        ...
        // Testing false
        const createFALSEParameters: { update: { emailVerified: number } } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                config: {
      ...
      it("should modify JSON type to TEXT if the DB doesn't support it. And expect the result to be transformed back to JSON", async () => {
        const createJSONParameters: { update: { preferences: string } } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter({
              config: {
                supportsJSON: false,
        ...
        expect(createJSONParameters.update.preferences).toEqual('{"color":"blue","size":"large"}')
      })

      it("should modify date type to TEXT if the DB doesn't support it. And expect the result to be transformed back to date", async () => {
        const testDate = new Date()
        const createDateParameters: { update: { createdAt: string } } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter({
              config: {
                supportsDates: false,
        ...
        expect(createDateParameters.update.createdAt).toEqual(testDate.toISOString())
      })
      ...
      it("should allow custom transform input", async () => {
        ...
        expect(createCustomTransformInputParameters.update).toHaveProperty("name")
        expect(createCustomTransformInputParameters.update.name).toEqual("TEST-NAME-2")
      })
      ...
      it("should allow custom transform output", async () => {
        ...
        expect(createCustomTransformOutputParameters.update).toHaveProperty("name")
        expect(createCustomTransformOutputParameters.update.name).toEqual("test-name-2")
      })
      ...
      it("should allow custom transform input and output", async () => {
        ...
        expect(createCustomTransformInputAndOutputParameters.update).toHaveProperty("name")
        expect(createCustomTransformInputAndOutputParameters.update.name).toEqual(
          "test-name-2".toUpperCase(),
        )
      })
    ...
    describe("find", () => {
      it("findOne: Should transform the where clause according to the schema", async () => {
        const parameters: { where: Where[]; model: string; select?: string[] } = await new Promise(
          (r) => {
            ;(async () => {
              const adapter = await createTestAdapter({
                options: {
      ...
      it("findMany: Should transform the where clause according to the schema", async () => {
        const parameters: { where: Where[] | undefined; model: string } = await new Promise((r) => {
          ;(async () => {
            const adapter = await createTestAdapter<BetterAuthOptions>({
              options: {
                user: {
