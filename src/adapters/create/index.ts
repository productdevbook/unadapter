  ...
  return (
    ...
    const getDefaultModelName = (model: string) => {
      ...
      if (config.usePlural && model.charAt(model.length - 1) === "s") {
        ...
        if (!m) {
          m = Object.entries(schema).find(([_, f]) => f.modelName === pluralessModel)?.[0]
        }
    ...
    const getDefaultFieldName = ({
      ...
      if (!f) {
        ...
        f = Object.values(schema[model]?.fields).find((f) => f.fieldName === field)
      }
      if (!f) {
    ...
    const getFieldAttributes = ({ model, field }: { model: string; field: string }) => {
    ...
    const transformOutput = async (
      ...
      for (const key in tableSchema) {
        ...
        if (field) {
          ...
          let newValue =
            data[
              Object.entries(newMappedKeys).find(([_, v]) => v === originalKey)?.[0] || originalKey
            ]
    ...
    return {
      create: async ({ data: unsafeData, model: unsafeModel, select }) => {
        ...
        const data = (await transformInput(unsafeData, unsafeModel, "create")) as T
        debugLog(
          { method: "create" },
      ...
      update: async ({ model: unsafeModel, where: unsafeWhere, update: unsafeData }) => {
        ...
        const data = (await transformInput(unsafeData, unsafeModel, "update")) as T
        debugLog(
          { method: "update" },
          `${formatTransactionId(thisTransactionId)} ${formatStep(2, 4)}`,
      ...
      findOne: async ({ model: unsafeModel, where: unsafeWhere, select }) => {
        ...
        const transformed = await transformOutput(res as any, unsafeModel, select)
        debugLog(
          { method: "findOne" },
