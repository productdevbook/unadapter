...
>(options: AdapterOptions<T, Schema>, schema: Schema) {
  ...
  return {
    transformInput(data: Record<string, any>, model: string, action: "create" | "update") {
  ...
  return (
    ...
    return {
      ...
      async findMany({ model, where, limit, offset, sortBy, select }) {
        ...
        if (sortBy) {
          cursor.sort(transform.getField(sortBy.field, model), sortBy.direction === "desc" ? -1 : 1)
        }
      ...
      async delete({ model, where }) {
        const clause = transform.convertWhereClause(where, model)
        await db.collection(transform.getModelName(model)).findOneAndDelete(clause)
      },
